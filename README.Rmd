...
author: "Robin Lovelace and Lex Comber"
output:
  github_document:
    toc: true
    toc_depth: 2
...
```{r, eval=FALSE, echo=FALSE}
sf::write_sf(zones_leeds, "zones_leeds.geojson")
```
...
# do not run because osmdata is sketchy
library(osmdata)
q = opq(bbox = 'leeds, uk') %>% 
...
piggyback::pb_upload("parks.geojson")
piggyback::pb_download_url("parks.geojson")
...
For advanced users: you can download, zip and move the files needed for the project with the following commands from a Unix shell:
...
zones_leeds = zones_west_yorkshire[zones_west_yorkshire$lad_name == "Leeds", ]
plot(zones_leeds)
write_sf(zones_leeds, "zones_leeds.geojson", delete_dsn = TRUE)
```
...
Instead of putting a buffer around *every* flow we will pick the top 50 lines (that have a distance) for clarity:
